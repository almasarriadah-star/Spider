...
1. **الكاميرا الحرارية (`camera.py → _uart_loop`)** — ✅ المفكّك مكتمل ومرن: مزامنة رأس +
   فك 768 بكسل + تحويل °C، مع تخطّي القمامة وإعادة المزامنة. **يُضبط من `sensors.json →
   thermal`** بثلاثة مفاتيح فقط حسب داتاشيت موديولك:
   - `header`: بايتات المزامنة hex (مثل `"5A5A"`؛ فارغ = إطارات ثابتة الطول بلا رأس).
   - `encoding`: `u16` (uint16 LE) | `u16be` (BE) | `f32` (float32 LE).
   - `scale`: للقسمة على القيمة الصحيحة لإخراج °C (يُتجاهَل مع f32).
   إن ظهرت الصورة مشوّشة/مقلوبة الحرارة فالأرجح `encoding` أو `scale` خطأ.

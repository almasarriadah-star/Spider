...
## 5) نقاط تحتاج إكمالاً (TODO — بروتوكولات يحدّدها المصنّع)

1. **`spider/sensors/soil.py` → `_loop`**: صيغة الرقم القادم عبر USB (خام أم نسبة).
   حالياً يفترض سطراً نصياً برقم خام. عدّل الفك حسب موديولك.
2. **`spider/sensors/camera.py` → `_uart_loop`**: فك إطار موديول الكاميرا الحرارية
   (رأس + ترتيب البايت + السرعة). يوجد مثال جاهز بالتعليقات. حالياً يقرأ بأمان دون
   تعبئة المصفوفة حتى يُكمَّل الفك.

أمّا **LD06** فمفكّكه مكتمل (رأس `0x54 0x2C`، 12 نقطة/حزمة، 47 بايت) ولا يحتاج تعديلاً
إلا إن اختلف الموديل.

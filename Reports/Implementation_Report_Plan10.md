...
**أُصلِح:**
- الـ UARTs الإضافية لم تكن تعمل: السبب الجذري **كومنتات عربية + أسهم `→` على نفس سطر
  `dtoverlay=`** منعت الـ overlays. الحل: تنظيف الأسطر (كومنت على سطر منفصل) + إعادة تشغيل
  → ظهرت `ttyAMA3/4/5`. (تبيّن أن **`disable-bt` اختياري** ولا يلزم لعملها.)
- **الأسماء الفعلية**: `uart3→ttyAMA3` (LD06) · `uart4→ttyAMA4` (GPS) · `uart5→ttyAMA5`
  (كاميرا). صُحِّح `sensors.json` تبعاً لذلك.
...
- **IMU**: يبقى على `serial0` (mini‑UART `ttyS0`، الإعداد الأصلي). جُرِّب أيضاً PL011
  (disable-bt) — **كلاهما 0 بايت** (انظر أدناه).

**يعمل حقيقي الآن:** LD06 (360 نقطة) · GPS (المنفذ، بانتظار fix) · غاز MQ135 · سيرفوات ·
كاميرا RGB · DHT22 (المكتبة جاهزة).

**معلّق على العتاد (قراءة خام = 0 بايت بعد إيقاف الخدمة — تشخيص قاطع):**
- **IMU (BNO085)**: الأقطاب 14/15 مؤكَّدة مربوطة بالـ UART (`pinctrl` = TXD/RXD)، والحسّاس
  **ليس** على I2C. ومع ذلك **0 بايت على GPIO15 على كلٍّ من mini‑UART و PL011** → الحسّاس
  لا يرسل أصلاً. السبب عتادي: تغذية، أو TX ليس على p10، أو **ليس بوضع UART‑RVC** (أقطاب PS).
- **الكاميرا الحرارية (`ttyAMA5`)**: المنفذ يفتح ونرسل أمر البدء، لكن **0 بايت** → تحقّق من

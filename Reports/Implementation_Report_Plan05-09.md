...
## 🔍 تدقيق التكامل (مراجعة ثانية — بعد سؤال "هيك صح؟")

المراجعة الأولى تحققت من **الاستيراد والترجمة فقط** — لم تتحقق أن نداءات الواجهة تطابق عقود الـ routes. التدقيق الثاني كشف **8 مشاكل تكامل حقيقية** وأُصلحت كلها:

| # | المشكلة | الإصلاح |
|---|---------|---------|
| 1 | `/api/move` ينتج `rotate_cw/ccw` والمحرّك يدعم `turn_left/turn_right` | تحويل لـ `turn_left/turn_right` |
| 2 | قائمة الحركات الـ23 بأسماء مخترعة (hi_five, twerk, ...) | استبدال بالـ23 الفعلية من `moves.py` |
| 3 | أسماء الجسم (`up/down/lean_fwd/tilt_*`) ≠ الفعلية | `body_up/body_down/lean_forward/twist_*` |
| 4 | `/api/health` بلا `hardware` | أُضيف `"hardware": HARDWARE` |
| 5 | `easing/set` يقرأ `type` والواجهة ترسل `easing` | الواجهة ترسل `{type}` وتقرأ `r.easing` |
| 6 | `/api/move` يتجاهل نمط المشي المختار | يقرأ `gait` ويطبّق الأنماط الأمامية |
| 7 | أزرار الجانبي تذهب لـ smooth-ripple | `startWalk()` → `forward/start` (lateral) |
| 8 | اتجاه الجانبي معكوس (vy>0 → يمين) | `vy>0 → shift_left` (يطابق الجوي ستيك) |

**تحقق ثابت نهائي (cross-check آلي):**
```
[SPECIALS] dashboard ↔ backend: 23/23 — كلها تطابق
[BODY]     dashboard ↔ backend: كلها تطابق
[gaOnce]   كل الأنواع صالحة في المحرّك
[startWalk] كل الأنواع صالحة في lateral_types
[GPS sim]  set_motion يحرّك الموقع فعلاً (أُزيل الاستيراد الدائري)
```

> ⚠️ درس: التحقق بـ `import` و`py_compile` لا يكفي لواجهة ↔ backend — يجب مطابقة كل اسم نوع/حركة ومعامل. القيود الحقيقية (cv2/عتاد) لا تظهر في الترجمة.

...
| [BUG03](BUG03_gait_restart_race.md) | المشي يعمل أول مرة فقط + الوقوف لا يعمل (سباق ملكية المحكّم) | 🔴 عالية | ✅ **حُلّ** |
| [BUG04](BUG04_estop_inconsistent.md) | الإيقاف الطارئ سلوكه مختلف كل مرة | 🟠 متوسطة-عالية | ✅ **حُلّ** |

> ⚠️ **BUG03 و BUG04 كانا سبب التراجع الذي أبلغ عنه المستخدم** بعد رفع آخر تحديث (المشي يعمل أول مرة فقط، الوقوف لا يعمل، الطوارئ عشوائي). كلاهما نتج عن **نظام ملكية المحكّم + الإيقاف غير المتزامن** الذي أُدخل في إعادة الهيكلة. **حُلّا في 2026-05-30** بجعل الإيقاف متزامناً (`thread.join`) + حارس خيط حيّ + ترتيب الطوارئ (قطع PWM أولاً) + تزامن علم الإيقاف. كل التغييرات في `web_controller.py` (لم نلمس منطق المشي).

### مقارنة بالنسخة قبل إعادة الهيكلة (لماذا حصل التراجع)
في النسخة القديمة (`16ed9df`) كانت `set_servo` **تكتب مباشرة على الهاردوير بلا نظام ملكية**، فلم يكن هناك تعارض بين «المشي» و«الوقوف». إعادة الهيكلة (`c8314c7`) أدخلت `MotionArbiter` بنظام `acquire/release` — وهو مفيد (كاتب وحيد + تحديد سرعة + منع تيار الذروة) لكن **تسليم الملكية كان غير متزامن**: روت الإيقاف يرجع قبل أن يحرّر الخيط ملكيته، فيرفض المحكّم طلب «الوقوف» (409). الإصلاح أبقى المحكّم وأصلح التسليم.
